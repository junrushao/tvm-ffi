...
class Param:
    before: S.Expr
    after: S.Expr
    bounds: Mapping[std.Var, tuple[int, int]] = dataclasses.field(default_factory=dict)

    def __init__(
        self,
        before: S.Expr | bool,
        after: S.Expr | int,
        bounds: Mapping[std.Var, tuple[int, int]] | None = None,
    ) -> None:
...
class TestCast(_Test):
    i = var("i", dtype="int32")
    j = var("j", dtype="int32")
    i_64 = var("i", dtype="int64")
    j_64 = var("j", dtype="int64")
...
class TestMinValue(_Test):
    ...
    param = (
        Param(S.equal(S.min_value("int32") - x, 0), S.equal(x, S.min_value("int32"))),
        Param(S.equal(S.min_value("int32") + x, 0), S.const("bool", False)),
        Param(S.equal(S.min_value("int32") - x, 0), S.equal(x, S.min_value("int32"))),
        Param(S.equal(S.min_value("int32") + x, 0), S.const("bool", False)),
        Param(S.equal(-x + S.min_value("int32"), 0), S.equal(x, S.min_value("int32"))),
        Param(S.equal(x + S.min_value("int32"), 0), S.const("bool", False)),
        Param(S.equal(-x + S.min_value("int32"), 0), S.equal(x, S.min_value("int32"))),
        Param(S.equal(x + S.min_value("int32"), 0), S.const("bool", False)),
    )
...
class TestLE(_Test):
    ...
    def _add_bound(self, analyzer: S.Analyzer, param: Param) -> None:
        for var, bounds in param.bounds.items():
            assert isinstance(var.ty, std.PrimTy)
...
class TestThreadBinding(_Test):
    x1 = var("x1", dtype="int32")
    x2 = var("x2", dtype="int32")
    tx = var("tx", dtype="int32")
    ty = var("ty", dtype="int32")
    vec = var("vec", dtype="int32")
