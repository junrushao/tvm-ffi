...
from tvm_ffi.std import floordiv as fld
from tvm_ffi.std import floormod as flm
...
class Param:
    ...
    def __init__(
        ...
        before: S.Expr | bool,
        after: S.Expr | int,
        constraints: S.Expr | list[S.Expr] | None = None,
    ) -> None:
...
class _Test:
    def test_body(self, param: Param) -> None:
        ...
        if not tvm_ffi.structural_equal(param.after, after):
...
def _const(value: int | float, dtype: str) -> S.Expr:
    if dtype == "bool":
        value = bool(value)
    return S.const(dtype=dtype, value=value)


def _i32(value: int) -> S.Expr:
    return S.const("int32", value)
...
class TestCancellation(_Test):
    var_int8 = var("var_int8", "int8")
    var_int32 = var("var_int32", "int32")
    var_int64 = var("var_int64", "int64")
    var_uint8 = var("var_uint8", "uint8")
    var_uint32 = var("var_uint32", "uint32")
    var_uint64 = var("var_uint64", "uint64")
    ...
    param = (
        ...
        Param(_eq(_const(5, "int64"), _const(5, "int64")), _const(True, "bool")),
        Param(_eq(_const(5, "uint8"), _const(5, "uint8")), _const(True, "bool")),
        Param(_eq(var_int8, var_int8), _const(True, "bool")),
        Param(_eq(var_int32, var_int32), _const(True, "bool")),
        Param(_eq(var_int64, var_int64), _const(True, "bool")),
        Param(_eq(var_uint8, var_uint8), _const(True, "bool")),
        Param(_eq(var_uint32, var_uint32), _const(True, "bool")),
        Param(_eq(var_uint64, var_uint64), _const(True, "bool")),
        Param(_ne(_const(5, "int64"), _const(5, "int64")), _const(False, "bool")),
        Param(_ne(_const(5, "uint8"), _const(5, "uint8")), _const(False, "bool")),
        Param(_ne(var_int8, var_int8), _const(False, "bool")),
        Param(_ne(var_int32, var_int32), _const(False, "bool")),
        Param(_ne(var_int64, var_int64), _const(False, "bool")),
        Param(_ne(var_uint8, var_uint8), _const(False, "bool")),
        Param(_ne(var_uint32, var_uint32), _const(False, "bool")),
        Param(_ne(var_uint64, var_uint64), _const(False, "bool")),
    )
...
class TestModIndex(_Test):
    ...
    y = var("y", dtype="int32")
    z = var("z", dtype="int32")
    nx = var("nx", dtype="int32")
    ny = var("ny", dtype="int32")
...
class TestComparisons(_Test):
    ...
    param = (
        ...
        Param(S.logical_and(_i32(0) <= x - y * 5, x - y * 5 < 5), _eq(y, fld(x, 5))),
        Param(S.logical_and(x - y * 5 < 5, _i32(0) <= x - y * 5), _eq(y, fld(x, 5))),
...
class TestCast(_Test):
    ...
    def _generate_tests() -> typing.Generator[Param, None, None]:
        ...
        for dtype1 in dtypes:
            ...
            yield Param(_eq(x, x).astype(dtype1), _const(1, dtype1))
