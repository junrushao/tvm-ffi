...
class Param:
    expr: S.Expr
    dom: dict[std.Var, IntervalSet]
    expected: tuple[S.Expr | int, S.Expr | int]
    bounds: dict[std.Var, tuple[int, int]] = dataclasses.field(default_factory=dict)
    analyzer_bounds: dict[std.Var, tuple[int, int]] = dataclasses.field(default_factory=dict)
...
def test_constructor() -> None:
    analyzer = S.Analyzer()
    s = IntervalSet(2, 3)
    assert analyzer.can_prove_equal(s.min_value, 2)
    assert analyzer.can_prove_equal(s.max_value, 3)

    s = IntervalSet(2, 2)
    assert analyzer.can_prove_equal(s.min_value, 2)
    assert analyzer.can_prove_equal(s.max_value, 2)
