...
The Python package is built with scikit-build-core, which drives CMake to compile the C++ core and Cython extension.

```bash
pip install --reinstall --verbose -e . --config-settings cmake.define.TVM_FFI_ATTACH_DEBUG_SYMBOLS=ON
```

The ``-e/--editable`` install reflects Python-only changes immediately, and the ``--reinstall`` flag forces a rebuild.
...
- Library not found at import time: ensure your dynamic loader can find the shared library. For source builds, add the ``lib`` directory to ``LD_LIBRARY_PATH`` (Linux), ``DYLD_LIBRARY_PATH`` (macOS), or ``PATH`` (Windows).
