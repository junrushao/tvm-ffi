...
def _stmt_names(body: list[std.Stmt]) -> list[str]:
    names = []
    for stmt in body:
        if isinstance(stmt, Instruction) and stmt.output is not None:
            names.append(stmt.output.name)
        else:
...
def test_dead_code_elimination_removes_unused_pure_instruction_chain() -> None:
    ...
    dead_add = AddInst(inputs=[x, x], output=dead_a)
    dead_mul = MulInst(inputs=[dead_a, x], output=dead_b)
    live_add = AddInst(inputs=[x, x], output=live_a)
    live_mul = MulInst(inputs=[live_a, x], output=live_b)
    store = StoreGlobalInst(inputs=[live_b, x])
...
def test_dead_code_elimination_preserves_unchanged_function_identity() -> None:
    ...
    x = _reg_var("x")
    store = StoreGlobalInst(inputs=[x, x])
...
def test_dead_code_elimination_rewrites_nested_statement_bodies() -> None:
    ...
    x = _reg_var("x")
    dead = AddInst(inputs=[x, x], output=_reg_var("dead"))
    live = AddInst(inputs=[x, x], output=_reg_var("live"))
...
def test_ir_rewriter_supports_delete_and_splice_in_statement_bodies() -> None:
    ...
    x = _reg_var("x")
    drop = AddInst(inputs=[x, x], output=_reg_var("drop"))
    keep = AddInst(inputs=[x, x], output=_reg_var("keep"))
