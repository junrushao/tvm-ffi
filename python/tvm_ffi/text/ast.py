...
class Node(Object):
    ...
    def to_python(self, config: PrinterConfig | None = None) -> str:
        ...
        """
        if config is None:
            from .printer import PrinterConfig  # noqa: PLC0415
...
@c_class("ffi.text.ast.StmtBlock", init=False)
...
@c_class("ffi.text.ast.Literal", init=False)
class Literal(Expr):
...
@c_class("ffi.text.ast.Id", init=False)
class Id(Expr):
    ...
    def __init__(self, name: str) -> None:
        self.__ffi_init__(name)


@c_class("ffi.text.ast.Attr", init=False)
class Attr(Expr):
    ...
    def __init__(self, obj: Expr, name: str) -> None:
        self.__ffi_init__(obj, name)


@c_class("ffi.text.ast.Index", init=False)
class Index(Expr):
    ...
    def __init__(self, obj: Expr, idx: Sequence[Expr]) -> None:
        self.__ffi_init__(obj, list(idx))


@c_class("ffi.text.ast.Call", init=False)
class Call(Expr):
    ...
    def __init__(
        ...
        self.__ffi_init__(callee, args, kwargs_keys, kwargs_values)
...
@c_class("ffi.text.ast.Operation", init=False)
class Operation(Expr):
    ...
    def __init__(self, op: int, operands: list[Expr]) -> None:
        self.__ffi_init__(op, operands)


@c_class("ffi.text.ast.Lambda", init=False)
class Lambda(Expr):
    ...
    def __init__(self, args: list[Id], body: Expr) -> None:
        self.__ffi_init__(args, body)


@c_class("ffi.text.ast.Tuple", init=False)
class Tuple(Expr):
    ...
    def __init__(self, values: list[Expr]) -> None:
        self.__ffi_init__(values)


@c_class("ffi.text.ast.List", init=False)
class List(Expr):
...
@c_class("ffi.text.ast.Dict", init=False)
class Dict(Expr):
...
@c_class("ffi.text.ast.If", init=False)
class If(Stmt):
    ...
    def __init__(self, cond: Expr, then_branch: list[Stmt], else_branch: list[Stmt]) -> None:
        self.__ffi_init__(cond, then_branch, else_branch)
...
class For(Stmt):
    ...
    def __init__(self, lhs: Expr, rhs: Expr, body: list[Stmt]) -> None:
        self.__ffi_init__(lhs, rhs, body)
...
class With(Stmt):
    ...
    def __init__(self, lhs: Expr | None, rhs: Expr, body: list[Stmt]) -> None:
        self.__ffi_init__(lhs, rhs, body)


@c_class("ffi.text.ast.ExprStmt", init=False)
class ExprStmt(Stmt):
...
@c_class("ffi.text.ast.Return", init=False)
class Return(Stmt):
    """A ``return`` statement.
...
class Function(Stmt):
    ...
    def __init__(
        ...
        self.__ffi_init__(name, args, decorators, return_type, body)
...
class Class(Stmt):
    ...
    def __init__(self, name: Id, decorators: list[Expr], body: list[Stmt]) -> None:
        self.__ffi_init__(name, decorators, body)
