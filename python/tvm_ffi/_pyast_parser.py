...
class Parser:
    ...
    def _run_generics(self, generic_name: str, operands: tuple[Any, ...]) -> Any:
        ...
        """
        handler = self._generic_by_type(generic_name, operands)
        if handler is not None:
            return handler(*operands)

        operand_dialects = {
            dialect
            for dialect in (_dialect_of(op) for op in operands if not self._is_literal_value(op))
    ...
    def _generic_by_type(
        self, generic_name: str, operands: tuple[Any, ...]
    ) -> Callable[..., Any] | None:
        """Find a generic handler from standard expression type MROs."""
        for op in operands:
            if not isinstance(op, std.Expr):
                continue
            for ty_cls in type(op.ty).__mro__:
                handler = self.generics.get((generic_name, ty_cls))
                if handler is not None:
                    return handler
        return None

    def _is_literal_value(self, value: Any) -> bool:
        """Detect values that should not force dialect dispatch.

        Native literals and language-specific immediate nodes are treated as
        neutral operands so expressions like ``x + 1`` dispatch on ``x``.
        """
        return isinstance(value, (*_NATIVE_LITERAL_TYPES, *_STD_LITERAL_TYPES))
    ...
    def visit_Operation(self, node: pyast.Operation) -> Any:
        ...
        if op in (kind.And, kind.Or):
            return self._fold_operation(_OP_GENERICS[op], operands)
        if op == kind.Parens:
    ...
    def _fold_operation(self, generic_name: str, operands: Sequence[pyast.Expr]) -> Any:
        """Evaluate left-associative boolean operations."""
        result = self.visit(operands[0])
        for operand in operands[1:]:
            result = self._run_generics(generic_name, (result, self.visit(operand)))
        return result
