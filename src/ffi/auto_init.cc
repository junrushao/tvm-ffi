 ...
 */
/*
 * \file src/ffi/auto_init.cc
 *
 * \brief Implementation of MakeAutoInit: builds a packed ``__ffi_init__``
 *        from pre-computed reflection field analysis.
 */
...
Function MakeAutoInit(int32_t type_index) {
  ...
  ObjectRef kwargs_sentinel = ffi::GetKwargsObject();
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def("ffi.MakeAutoInit",
                        [](int32_t type_index) -> Function { return MakeAutoInit(type_index); });
}
