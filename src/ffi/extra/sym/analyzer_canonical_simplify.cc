...
inline Expr ModImpl(const Expr& a, const Expr& b, DivMode mode) {
...
inline Expr DivImpl(const Expr& a, const Expr& b, DivMode mode) {
  if (mode == DivMode::kTruncDiv) {
    return truncdiv(a, b);
  } else {
    return floordiv(a, b);
...
bool CastIsSafe(DLDataType dtype, const Expr& value, AnalyzerObj::Impl* analyzer) {
  if (!IsIndexType(dtype)) {
    return false;
  }
  ConstIntBound bound = analyzer->const_int_bound(value);
  int64_t ubound = max_value(dtype).as<IntImmObj>()->value;
  int64_t lbound = min_value(dtype).as<IntImmObj>()->value;
  if (DTypeOf(value).bits <= dtype.bits ||  // upcast is safe
...
Expr SplitExprObj::NormalizeWithScale(int64_t sscale) const {
  Expr res = this->index;
  // DLDataType dtype = DTypeOf(this);
  if (this->scale == 0) {  // NOLINT(clang-analyzer-core.UndefinedBinaryOperatorResult)
    return Const(DTypeOf(this), 0);
  }
  if (this->upper_factor != kPosInf) {
    res = ModImpl(res, Const(DTypeOf(this), this->upper_factor), div_mode);
  }
  if (this->lower_factor != 1) {
    res = DivImpl(res, Const(DTypeOf(this), this->lower_factor), div_mode);
  }
  sscale *= this->scale;
  if (sscale != 1) {
    // TODO(junrus): recover this
    // ICHECK(!DTypeOf(this).is_uint() || sscale > 0);
    res = res * sscale;
  }
  return res;
}
...
bool SplitExprObj::CanPushCastToChildren(DLDataType dtype_, AnalyzerObj::Impl* analyzer) const {
  ...
  if (dtype_.bits >= DTypeOf(this).bits) {
  ...
  if (this->scale != 1) {
    // TODO(junrus): recover
    // ICHECK(!DTypeOf(this).is_uint() || this->scale > 0);
    res = res * this->scale;
    if (!CastIsSafe(dtype_, res, analyzer)) {
      return false;
    }
...
Expr SumExprObj::Normalize() const {
  // quick path 1.
  if (this->args.size() == 0) {
    return Const(DTypeOf(this), this->base);  // NOLINT(clang-analyzer-core.CallAndMessage)
  }
  // NOLINTNEXTLINE(clang-analyzer-core.CallAndMessage)
  return Normalize_(DTypeOf(this), SimplifySplitExprs(args), base);
}
...
void SumExprObj::DivideBy(int64_t scale) {
  // TODO(junrus): recover
  // ICHECK_equal(this->base % scale, 0);
  this->base /= scale;
  for (size_t i = 0; i < this->args.size(); ++i) {
    // TODO(junrus): recover
    // ICHECK_equal(args[i]->scale % scale, 0);
    args[i].CopyOnWrite()->scale /= scale;
  }
}
...
void SumExprObj::AddToSelf(SplitExpr other, int64_t scale) {
  ...
  for (size_t j = start; j < args.size(); ++j) {
    if (!args[j]->IndexEqual(other) || other->lower_factor > args[j]->lower_factor) {
      ...
      using DifferenceType = std::vector<SplitExpr>::difference_type;
      this->args.insert(this->args.begin() + static_cast<DifferenceType>(j), other);
      return;
    }
...
bool SumExprObj::CanPushCastToChildren(DLDataType dtype_, AnalyzerObj::Impl* analyzer) const {
  ...
  Expr res = Const(dtype_, 0);
  for (const auto& arg : args) {
    if (arg->scale > 0) {
      res = res + arg->Normalize();
      if (!CastIsSafe(dtype_, res, analyzer)) {
        return false;
      }
    }
  ...
  for (const auto& arg : args) {
    if (arg->scale < 0) {
      res = res - arg->NormalizeWithScale(-1);
      if (!CastIsSafe(dtype_, res, analyzer)) {
        return false;
      }
    }
...
struct CanonicalSimplifier::Impl : public RewriteSimplifier::Impl {
  ...
  Expr CanonicalMutate(const Expr& expr) { return Rewriter::VisitExpr(expr); }
  Expr VisitExpr_(const AddObj* op) final;
  Expr VisitExpr_(const SubObj* op) final;
  Expr VisitExpr_(const MulObj* op) final;
  Expr VisitExpr_(const CDivObj* op) final;
  Expr VisitExpr_(const CModObj* op) final;
 ...
 private:
  ...
  SplitExpr ToSplitExpr(Expr expr) {
    if (auto op = expr.as<SplitExprObj>()) {
      return RefFromPtr<SplitExpr>(op);
    }
    if (const auto* op = expr.as<SumExprObj>()) {
      if (op->base == 0 &&  // NOLINT(clang-analyzer-core.UndefinedBinaryOperatorResult)
          op->args.size() == 1) {
        return op->args[0];
      }
    }
    ...
    return SplitExpr(DTypeOf(expr), expr);
  }
  /*!
   ...
   */
  SumExpr ToSumExpr(const Expr& expr) {
    if (auto op = expr.as<SumExprObj>()) {
      return RefFromPtr<SumExpr>(op);
    }
    SumExpr n(DTypeOf(expr));
...
Expr CanonicalSimplifier::Impl::VisitExpr_(const AddObj* op) {
  ...
  // const folding
  if (auto const_res = TryConstFold<AddObj>(a, b)) return const_res.value();

  // canonical form simplification.
  SumExpr ret = ToSumExpr(a);
...
Expr CanonicalSimplifier::Impl::VisitExpr_(const SubObj* op) {
  ...
  // const folding
  if (auto const_res = TryConstFold<SubObj>(a, b)) return const_res.value();
...
Expr CanonicalSimplifier::Impl::VisitExpr_(const MulObj* op) {
  ...
  // const folding
  if (auto const_res = TryConstFold<MulObj>(a, b)) return const_res.value();
...
void CanonicalSimplifier::Impl::SeparateDivisibleParts(const SumExprObj* psum, int64_t coeff,
                                                       SumExpr* out_divisible,
                                                       SumExpr* out_non_divisible) {
  SumExpr divisible(DTypeOf(psum));
  SumExpr non_divisible(DTypeOf(psum));
...
SplitExpr CanonicalSimplifier::Impl::SplitDivConst(SplitExpr lhs, int64_t cval, DivMode div_mode) {
  ...
  if (cval % lhs->scale == 0) {
    ...
    if (lhs->upper_factor == kPosInf ||
      ...
      return ToSplitExpr(Const(DTypeOf(lhs), 0));
    } else {
      // move the upper_factor modular into index.
      lhs.CopyOnWrite()->index =
          ModImpl(lhs->index, Const(DTypeOf(lhs), lhs->upper_factor), div_mode);
...
bool CanonicalSimplifier::Impl::ProdDivSimplify(Expr* plhs, Expr* prhs, Expr* common_scale) {
  ...
  if (prhs->as<IntImmObj>()) return false;
  // collect lhs products and try to eliminate by matching them to prod in rhs
  // TODO(junrus): Support List<Optional<>>
  std::vector<Optional<Expr>> lhs_prods;
  Expr new_rhs = Const(DTypeOf(*prhs), 1);
  Expr new_common_scale = Const(DTypeOf(*prhs), 1);
  int64_t lhs_cscale = 1, rhs_cscale = 1;
  int num_elimination = 0;

  // collect lhs product and constant scale.
  auto fcollect_lhs = [&](const Expr& value) {
    if (auto* intimm = value.as<IntImmObj>()) {
      lhs_cscale *= intimm->value;
    } else {
      lhs_prods.emplace_back(value);
    }
  };
  ...
  auto fcollect_rhs = [&](const Expr& value) {
    if (auto* intimm = value.as<IntImmObj>()) {
      rhs_cscale *= intimm->value;
    } else {
      // try eliminate from lhs
      for (auto& lhs_prod : lhs_prods) {
        if (lhs_prod.has_value()) {
          if (deep_equal(value, lhs_prod.value())) {
            lhs_prod = std::nullopt;
            ++num_elimination;
  ...
  Expr new_lhs = Const(DTypeOf(*plhs), 1);
  for (const Optional<Expr>& val : lhs_prods) {
...
Expr CanonicalSimplifier::Impl::VisitExpr_(const CDivObj* op) {
  if (!IsIndexType(DTypeOf(op))) {
    return Rewriter::VisitExpr_(op);
  }
  ...
  // const folding
  if (auto const_res = TryConstFold<CDivObj>(a, b)) return const_res.value();
  PVar<IntImm> c1;
  // x / c1
      ...
      if (analyzer_->CanProveGreaterEqual(lhs->Normalize(), 0) &&
        ...
        if (const auto* pconst = temp.as<IntImmObj>()) {
          ...
          if (TryCompare(temp, cval) != CompareResult::kLT) {
            lhs.CopyOnWrite()->AddToSelf(SplitDivConst(ToSplitExpr(temp), cval, DivMode::kTruncDiv),
                                         1);
          }
        }
        return lhs;
...
Expr CanonicalSimplifier::Impl::VisitExpr_(const FloorDivObj* op) {
  ...
  // const folding
  if (auto const_res = TryConstFold<FloorDivObj>(a, b)) return const_res.value();
  PVar<IntImm> c1;
  // x / c1
  if (c1.Match(b) && c1.Eval()->value > 0) {
    int64_t cval = c1.Eval()->value;
    ...
    if (const auto* psum = a.as<SumExprObj>()) {
      ...
      if (const auto* pconst = temp.as<IntImmObj>()) {
        ...
        if (!(TryCompare(temp, cval) == CompareResult::kLT &&
          ...
          lhs.CopyOnWrite()->AddToSelf(SplitDivConst(ToSplitExpr(temp), cval, DivMode::kFloorDiv),
                                       1);
        }
      }
      return lhs;
...
SplitExpr CanonicalSimplifier::Impl::SplitModConst(SplitExpr lhs, int64_t cval, DivMode div_mode) {
    ...
    if (lhs->upper_factor == kPosInf || lhs->upper_factor % new_upper_factor == 0) {
      ...
      if (new_upper_factor < lhs->upper_factor && lhs->upper_factor != kPosInf) {
        auto updated = ToSplitExpr(
            this->VisitExpr(ModImpl(lhs->index, Const(DTypeOf(lhs), new_upper_factor), div_mode)));
  ...
  // ICHECK(lhs->DivModeCompatibleTo(div_mode));
  // ICHECK_equal(lhs->scale, 1);
  // ICHECK_equal(lhs->lower_factor, 1);
  lhs.CopyOnWrite()->div_mode = div_mode;
  lhs.CopyOnWrite()->upper_factor = cval;
  return lhs;
}

Expr CanonicalSimplifier::Impl::VisitExpr_(const CModObj* op) {
  if (!IsIndexType(DTypeOf(op))) {
    return Rewriter::VisitExpr_(op);
  }
  // normalize
  ...
  // const folding
  if (auto const_res = TryConstFold<CModObj>(a, b)) return const_res.value();
...
Expr CanonicalSimplifier::Impl::VisitExpr_(const FloorModObj* op) {
  ...
  // const folding
  if (auto const_res = TryConstFold<FloorModObj>(a, b)) return const_res.value();
...
Expr CanonicalSimplifier::Impl::VisitExpr_(const CastObj* op) {
  ...
  if (value.as<SumExprObj>()) {
    ...
    if (se->CanPushCastToChildren(DTypeOf(op), analyzer_)) {
      se.CopyOnWrite()->PushCastToChildren(DTypeOf(op));
      return se;
    }
  }
...
Expr CanonicalSimplifier::Impl::VisitExpr_(const LtObj* op) {
  ...
  if (const auto* lhs = expr.as<SumExprObj>()) {
    ...
    // TODO(junrus): recover this
    // ICHECK(DTypeOf(extra) == DTypeOf(divisible));
    Expr normal_extra = extra->Normalize();
    if (this->analyzer_->CanProve(normal_extra < gcd) &&
        this->analyzer_->CanProve(normal_extra > -gcd)) {
      // Case 1. -d < xn < d
...
CanonicalSimplifier::CanonicalSimplifier(AnalyzerObj::Impl* parent)
