 ...
 * \tparam Operator The integer operator.
 */
template <typename Operator>
inline bool WillOverflow([[maybe_unused]] int64_t x, [[maybe_unused]] int64_t y,
                         [[maybe_unused]] int64_t min_value, [[maybe_unused]] int64_t max_value) {
  return false;
}

template <>
inline bool WillOverflow<AddObj>(int64_t x, int64_t y, int64_t min_value, int64_t max_value) {
...
template <>
inline bool WillOverflow<SubObj>(int64_t x, int64_t y, int64_t min_value, int64_t max_value) {
  if ((y > 0) && (x < min_value + y)) return true;
  if ((y < 0) && (x > max_value + y)) return true;
  return false;
}
...
template <>
inline bool WillOverflow<MulObj>(int64_t x, int64_t y, int64_t min_value, int64_t max_value) {
  if (y == 0) return false;
  if (y > 0) {
...
template <>
inline bool WillOverflow<CModObj>([[maybe_unused]] int64_t x, int64_t y,
                                  [[maybe_unused]] int64_t min_value,
...
struct ConstIntBoundAnalyzer::Impl : public ExprFunctor<ConstIntBoundAnalyzerEntry(const Expr&)> {
 public:
  using Base = ExprFunctor<ConstIntBoundAnalyzerEntry(const Expr&)>;
  ...
  struct BoundInfo {
    ...
    BoundInfo(Expr expr, ConstIntBoundAnalyzerEntry bound) : expr(std::move(expr)), bound(bound) {}
  };

  void Bind(const Var& var, const Range& range, bool allow_override) {
    ConstIntBoundAnalyzerEntry a = VisitExpr(RangeMin(range));
    ConstIntBoundAnalyzerEntry b = VisitExpr(RangeExtent(range));
  ...
  void Update(const Var& var, const ConstIntBoundAnalyzerEntry& info, bool allow_override) {
    if (!allow_override) {
      ...
      if (it != var_map_.end()) {
        ...
        //                            << "original=" << ConstIntBound(it->second.min_value,
        //                            it->second.max_value)
  ...
  ConstIntBoundAnalyzerEntry VisitExprDefault_(const Expr& op) final {
    return Everything(DTypeOf(op));
  }

  ConstIntBoundAnalyzerEntry VisitExpr(const Expr& expr) final {
    ConstIntBoundAnalyzerEntry res = Base::VisitExpr(expr);
    ExprDeepEqual equal;
    ...
    if (bound_) {
      ...
      if (it != bound_->end()) {
        ConstIntBoundAnalyzerEntry everything = Everything(DTypeOf(expr));
  ...
  ConstIntBoundAnalyzerEntry VisitExpr_(const CastObj* op) final {
    // TODO(junrus): recover ceil(log2(n))
    ConstIntBoundAnalyzerEntry a = VisitExpr(op->value);
    ConstIntBoundAnalyzerEntry b = Everything(DTypeOf(op));
  ...
  ConstIntBoundAnalyzerEntry VisitExpr_(const IntImmObj* op) final {
  ...
  ConstIntBoundAnalyzerEntry VisitExpr_(const CDivObj* op) final {
    ConstIntBoundAnalyzerEntry a = VisitExpr(op->a);
    ConstIntBoundAnalyzerEntry b = AssumeNoZeroDivisor(VisitExpr(op->b));
    return HandleDivision(a, b, DTypeOf(op), InfAwareDiv);
  }

  ConstIntBoundAnalyzerEntry VisitExpr_(const CModObj* op) final {
    ConstIntBoundAnalyzerEntry a = VisitExpr(op->a);
    ConstIntBoundAnalyzerEntry b = AssumeNoZeroDivisor(VisitExpr(op->b));
    ...
    if (b.min_value > 0) {
      ...
      if (a.min_value >= 0) {
        ...
        return MakeBound(std::max(a.min_value, -b_max_cap),
                         std::min(std::max(a.max_value, static_cast<int64_t>(0)), b_max_cap));
      }
    } else {
      // TODO(junrus): recover this
  ...
  ConstIntBoundAnalyzerEntry VisitExpr_(const FloorDivObj* op) final {
    ConstIntBoundAnalyzerEntry a = VisitExpr(op->a);
    ConstIntBoundAnalyzerEntry b = AssumeNoZeroDivisor(VisitExpr(op->b));
    return HandleDivision(a, b, DTypeOf(op), InfAwareFloorDiv);
  }
  ...
  ConstIntBoundAnalyzerEntry VisitExpr_(const IfExprObj* op) final {
    ConstIntBoundAnalyzerEntry a = VisitExpr(op->then_expr);
    ConstIntBoundAnalyzerEntry b = VisitExpr(op->else_expr);
    return Union(a, b);
  }

  ConstIntBoundAnalyzerEntry VisitExpr_(const CallObj* op) final { return Everything(DTypeOf(op)); }

  ConstIntBoundAnalyzerEntry VisitExpr_(const VarObj* op) final {
    Var v = RefFromPtr<Var>(op);
  ...
  ConstIntBoundAnalyzerEntry VisitExpr_(const LShiftObj* op) final {
    ConstIntBoundAnalyzerEntry a = VisitExpr(op->a);
    ConstIntBoundAnalyzerEntry b = VisitExpr(op->b);
    if (a.min_value < 0 || b.min_value < 0) {
      return Everything(DTypeOf(op));
  ...
  ConstIntBoundAnalyzerEntry VisitExpr_(const RShiftObj* op) final {
    ConstIntBoundAnalyzerEntry a = VisitExpr(op->a);
    ConstIntBoundAnalyzerEntry b = VisitExpr(op->b);
    return BinaryOpBoundary(a, b, InfAwareRightShift);
  }
  ...
  ConstIntBoundAnalyzerEntry VisitExpr_(const BitwiseAndObj* op) final {
    ConstIntBoundAnalyzerEntry a = VisitExpr(op->a);
    ConstIntBoundAnalyzerEntry b = VisitExpr(op->b);
    if (a.min_value >= 0 && b.min_value >= 0) {
      return MakeBound(0, std::min(a.max_value, b.max_value));
  ...
  std::function<void()> EnterConstraint(const Expr& constraint) {
    ...
    auto frecover = [old_size, new_size, this]() {
      // TODO(junrus): recover this
      (void)new_size;
      //   ICHECK_equal(additional_info_.size(), new_size);
      using DifferenceType = std::vector<BoundInfo>::difference_type;
      additional_info_.erase(additional_info_.begin() + static_cast<DifferenceType>(old_size),
                             additional_info_.end());
    };
    return frecover;
  }

  // internal variable map
  std::unordered_map<Var, ConstIntBoundAnalyzerEntry, ObjectPtrHash, ObjectPtrEqual> var_map_;
   ...
   */
  template <typename F>
  static ConstIntBoundAnalyzerEntry BinaryOpBoundary(ConstIntBoundAnalyzerEntry a,
                                                     ConstIntBoundAnalyzerEntry b, const F& op) {
    ConstIntBoundAnalyzerEntry ret;
    ...
    ret.min_value = std::min({v1, v2, v3, v4});
    ret.max_value = std::max({v1, v2, v3, v4});
    return ret;
  }
  /*!
   * \brief Get value boundaries of division (e.g. CDiv or FloorDiv).
   * \param a The entry of the left operand.
   * \param b The entry of the right operand.
   ...
   */
  template <typename F>
  static ConstIntBoundAnalyzerEntry HandleDivision(ConstIntBoundAnalyzerEntry a,
                                                   ConstIntBoundAnalyzerEntry b, DLDataType dt,
                                                   const F& op) {
    // Here we have a / b.
    ...
    if (b.min_value <= 0 && 0 <= b.max_value && dt.code == kDLInt) {
      ConstIntBoundAnalyzerEntry b_neg =
          b.min_value < 0 ? MakeBound(b.min_value, -1) : Everything(dt);
      ConstIntBoundAnalyzerEntry b_pos =
          b.max_value > 0 ? MakeBound(1, b.max_value) : Everything(dt);
  ...
  static int64_t InfAwareMul(int64_t x, int64_t y) {
    if (!WillOverflow<MulObj>(x, y, kNegInf, kPosInf)) return x * y;
  ...
  static int64_t InfAwareDiv(int64_t x, int64_t y) {
    if (y == 0) {
      TVM_FFI_THROW(ValueError) << "Division by zero";
    }
   ...
   */
  static ConstIntBoundAnalyzerEntry Union(ConstIntBoundAnalyzerEntry a,
                                          ConstIntBoundAnalyzerEntry b) {
    ConstIntBoundAnalyzerEntry ret;
    ret.min_value = std::min(a.min_value, b.min_value);
    ret.max_value = std::max(a.max_value, b.max_value);
    return ret;
   ...
   * \param b the right operand.
   */
  static ConstIntBoundAnalyzerEntry Intersect(ConstIntBoundAnalyzerEntry a,
                                              ConstIntBoundAnalyzerEntry b) {
    ConstIntBoundAnalyzerEntry ret;
    ret.min_value = std::max(a.min_value, b.min_value);
    ret.max_value = std::min(a.max_value, b.max_value);
  ...
  static std::vector<BoundInfo> DetectBoundInfo(const Expr& cond) {
    ...
    auto add_info = [&](const Expr& expr, int64_t min_value, int64_t max_value) {
      ...
      if (!expr->IsInstance<IntImmObj>()) {
        info.emplace_back(expr, MakeBound(min_value, max_value));
      }
    };
...
ConstIntBoundAnalyzer::ConstIntBoundAnalyzer(AnalyzerObj::Impl*)
