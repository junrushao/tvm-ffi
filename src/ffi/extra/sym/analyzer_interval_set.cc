...
IntervalSet IntervalSetObj::Intersect(const IntervalSetObj* b, AnalyzerObj::Impl* analyzer) const {
  ...
  if (int_or_uint(DTypeOf(max_value_)) &&  //
      int_or_uint(DTypeOf(min_value_)) &&  //
...
IntervalSet IntervalSetObj::Union(const IntervalSetObj* b, AnalyzerObj::Impl*) const {
  if (this->IsEmpty()) return IntervalSet(b);
  if (b->IsEmpty()) return IntervalSet(this);
...
IntervalSet IntervalSet::FromRange(const Range& range) {
  if (IsConstInt(RangeExtent(range), 1)) {
    return SinglePoint(RangeMin(range));
  }
  Expr range_max = RangeExtent(range) + RangeMin(range);
  range_max = range_max - 1;
  return IntervalSet(RangeMin(range), range_max);
}
IntervalSet IntervalSet::Interval(const Expr& min, const Expr& max) {
  if (min.same_as(max)) {
...
IntervalSet IntersectIntervalSets(const List<IntervalSet>& sets, AnalyzerObj::Impl* analyzer) {
  if (sets.size() == 0) return IntervalSet::Nothing();
  if (sets.size() == 1) return sets[0];
  IntervalSet x = sets[0];
  for (int64_t i = 1, n = static_cast<int64_t>(sets.size()); i < n; ++i) {
...
inline constexpr bool kIsLogicalOrComparison =
    std::is_same_v<OpObj, EqObj> || std::is_same_v<OpObj, NeObj> || std::is_same_v<OpObj, LtObj> ||
    std::is_same_v<OpObj, LeObj> || std::is_same_v<OpObj, GtObj> || std::is_same_v<OpObj, GeObj> ||
    std::is_same_v<OpObj, AndObj> || std::is_same_v<OpObj, OrObj>;

template <typename Operator>
inline IntervalSet Combine(AnalyzerObj::Impl*, IntervalSet a, IntervalSet b, DLDataType dtype) {
  using OpObj = typename Operator::ContainerType;
  if (a->IsSinglePoint() && b->IsSinglePoint()) {
    Expr expr;
    if (auto res = TryConstFold<OpObj>(a->min_value, b->min_value)) {
      expr = res.value();
    } else {
      expr = BinaryOpMaker<OpObj>::Make(a->min_value, b->min_value);
    }
    return IntervalSet::SinglePoint(expr);
  }
  if constexpr (kIsLogicalOrComparison<OpObj>) {
    return IntervalSet(Const(dtype, 0), Const(dtype, 1));
  }
...
template <>
inline IntervalSet Combine<Add>(AnalyzerObj::Impl*, IntervalSet a, IntervalSet b, DLDataType) {
  if (a->IsSinglePoint() && b->IsSinglePoint()) {
...
template <>
inline IntervalSet Combine<Sub>(AnalyzerObj::Impl*, IntervalSet a, IntervalSet b, DLDataType) {
  if (a->IsSinglePoint() && b->IsSinglePoint()) {
    return IntervalSet::SinglePoint(a->min_value - b->min_value);
  }
  if (a->IsEmpty()) return a;
...
template <>
inline IntervalSet Combine<Mul>(AnalyzerObj::Impl* analyzer, IntervalSet a, IntervalSet b,
                                DLDataType) {
  if (a->IsSinglePoint() && b->IsSinglePoint()) {
    return IntervalSet::SinglePoint(a->min_value * b->min_value);
  }
  ...
  if (b->IsSinglePoint()) {
    if (IsConstInt(b->min_value, 0)) return b;
    if (IsConstInt(b->min_value, 1)) return a;
...
template <>
inline IntervalSet Combine<CDiv>(AnalyzerObj::Impl* analyzer, IntervalSet a, IntervalSet b,
                                 DLDataType) {
  if (a->IsSinglePoint() && b->IsSinglePoint()) {
  ...
  if (b->IsSinglePoint()) {
    if (IsConstInt(b->min_value, 0)) {
      TVM_FFI_THROW(ValueError) << "Divide by zero in CombineInterval CDiv";
    }
    if (IsConstInt(b->min_value, 1)) return a;
...
template <>
inline IntervalSet Combine<CMod>(AnalyzerObj::Impl* analyzer, IntervalSet a, IntervalSet b,
                                 DLDataType) {
  if (a->IsSinglePoint() && b->IsSinglePoint()) {
  ...
  if (b->IsSinglePoint()) {
    ...
    if (IsConstInt(divisor, 0)) {
      TVM_FFI_THROW(ValueError) << "Modular by zero in CombineInterval CMod";
    }
    ...
    if (analyzer->CanProveGreaterEqual(divisor, 0)) {
      return IntervalSet(Const(DTypeOf(divisor), 0), divisor - 1);
    } else {
...
template <>
inline IntervalSet Combine<FloorDiv>(AnalyzerObj::Impl* analyzer, IntervalSet a, IntervalSet b,
                                     DLDataType) {
  if (a->IsSinglePoint() && b->IsSinglePoint()) {
...
template <>
inline IntervalSet Combine<FloorMod>(AnalyzerObj::Impl* analyzer, IntervalSet a, IntervalSet b,
                                     DLDataType) {
  if (a->IsSinglePoint() && b->IsSinglePoint()) {
  ...
  if (b->IsSinglePoint()) {
    ...
    if (analyzer->CanProveGreaterEqual(divisor, 0)) {
      if (divisor->IsInstance<IntImmObj>()) {
        ...
        bool compatible_dtypes =
            !((DTypeOf(qmin).code == kDLOpaqueHandle) ^ (DTypeOf(qmax).code == kDLOpaqueHandle));
        if (compatible_dtypes && analyzer->CanProveEqual(qmax, qmin)) {
...
template <>
inline IntervalSet Combine<Max>(AnalyzerObj::Impl*, IntervalSet a, IntervalSet b, DLDataType) {
  if (a->IsSinglePoint() && b->IsSinglePoint()) {
...
template <>
inline IntervalSet Combine<Min>(AnalyzerObj::Impl*, IntervalSet a, IntervalSet b, DLDataType) {
  if (a->IsSinglePoint() && b->IsSinglePoint()) {
...
struct IntervalSetEvaluator : public ExprFunctor<IntervalSet(const Expr&)> {
  IntervalSetEvaluator(AnalyzerObj::Impl* analyzer, const Dict<Var, IntervalSet>& dom_map,
                       const std::vector<std::pair<Var, IntervalSet>>* dom_constraints = nullptr,
                       bool eval_vec = false)
      : analyzer_(analyzer), dom_map_(dom_map), dom_constraints_(dom_constraints) {
    (void)eval_vec;
  }
  ...
  IntervalSet VisitExpr_(const IntImmObj* op) final {
    return IntervalSet::SinglePoint(RefFromPtr<Expr>(op));
  }

  IntervalSet VisitExpr_(const VarObj* op) final {
    Var var = RefFromPtr<Var>(op);
    ...
    IntervalSet res = [&]() {
      if (values.size() == 1) {
        ...
        return IntersectIntervalSets(values, this->analyzer_);
      }
    }();
  ...
  IntervalSet VisitExpr_(const CDivObj* op) final { return VisitBinaryExpr_<CDiv>(op); }

  IntervalSet VisitExpr_(const CModObj* op) final { return VisitBinaryExpr_<CMod>(op); }
  ...
  IntervalSet VisitExpr_(const IfExprObj* op) final {
    IntervalSet true_set = this->Eval(op->then_expr);
    IntervalSet false_set = this->Eval(op->else_expr);
  ...
  IntervalSet VisitExpr_(const CastObj* op) final {
    ...
    if (value_set->min_value.same_as(value_set->max_value)) {
      ...
      return IntervalSet::SinglePoint(cast(DTypeOf(op), value_set->min_value));
    }
    Expr min_value =
        value_set->HasLowerBound() ? cast(DTypeOf(op), value_set->min_value) : neg_inf();
    Expr max_value =
        value_set->HasUpperBound() ? cast(DTypeOf(op), value_set->max_value) : pos_inf();
    return IntervalSet(min_value, max_value);
  }

  IntervalSet VisitExpr_(const CallObj*) final { return IntervalSet::Everything(); }
  IntervalSet VisitExprDefault_(const Expr&) final { return IntervalSet::Everything(); }
 ...
 private:
  ...
  inline IntervalSet VisitBinaryExpr_(const T* op) {
    static_assert(std::is_same_v<typename TOp::ContainerType, T>, "constraint");
    IntervalSet a = this->Eval(op->a);
    IntervalSet b = this->Eval(op->b);
    if (MatchPoint(a, op->a) && MatchPoint(b, op->b)) {
      return IntervalSet::SinglePoint(RefFromPtr<Expr>(op));
    }
    return Combine<TOp>(analyzer_, a, b, DTypeOf(op));
  }

  // recursive depth
  size_t recur_depth_{0};
  // analyzer
...
std::function<void()> IntervalSetAnalyzer::Impl::EnterConstraint(const Expr& constraint) {
  ...
  auto frecover = [old_size, new_size, this]() {
    ...
    using DifferenceType = std::vector<std::pair<Var, IntervalSet>>::difference_type;
    dom_constraints_.erase(dom_constraints_.begin() + static_cast<DifferenceType>(old_size),
                           dom_constraints_.end());
  };
  return frecover;
}

std::vector<std::pair<Var, IntervalSet>> IntervalSetAnalyzer::Impl::DetectBoundInfo(
  ...
  for (const Expr& subconstraint : ExtractConstraints(constraint)) {
    if ((x <= limit).Match(subconstraint)) {
      bounds.emplace_back(x.Eval(), IntervalSet::Interval(SymbolicLimits::neg_inf_, limit.Eval()));
    } else if ((x < limit).Match(subconstraint)) {
      bounds.emplace_back(x.Eval(),
                          IntervalSet::Interval(SymbolicLimits::neg_inf_, limit.Eval() - 1));
    } else if ((x >= limit).Match(subconstraint)) {
      bounds.emplace_back(x.Eval(), IntervalSet::Interval(limit.Eval(), SymbolicLimits::pos_inf_));
    } else if ((x > limit).Match(subconstraint)) {
      bounds.emplace_back(x.Eval(),
                          IntervalSet::Interval(limit.Eval() + 1, SymbolicLimits::pos_inf_));
    } else if ((x == limit).Match(subconstraint)) {
      bounds.emplace_back(x.Eval(), IntervalSet::SinglePoint(limit.Eval()));
    }
...
IntervalSetAnalyzer::IntervalSetAnalyzer(AnalyzerObj::Impl* parent)
    : impl_(std::make_unique<Impl>(parent)) {}
IntervalSetAnalyzer::~IntervalSetAnalyzer() = default;
IntervalSet IntervalSetAnalyzer::operator()(const Expr& expr,
                                            const Dict<Var, IntervalSet>& dom_map) {
