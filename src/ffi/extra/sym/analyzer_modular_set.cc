...
struct ModularSetAnalyzerEntry {
  ...
  bool operator==(const ModularSetAnalyzerEntry& other) const {
...
struct ModularSetAnalyzer::Impl : public ExprFunctor<ModularSetAnalyzerEntry(const Expr&)> {
 public:
  ...
  void Update(const Var& var, const ModularSet& info, bool allow_override) {
    if (!allow_override) {
      ...
      if (it != var_map_.end()) {
        ...
        //                            << "original=" << ModularSet(it->second.coeff,
        //                            it->second.base) << ", new=" << info;
      }
    }
  ...
  ModularSetAnalyzerEntry VisitExprDefault_(const Expr&) final { return Everything(); }

  ModularSetAnalyzerEntry VisitExpr_(const CastObj* op) final { return VisitExpr(op->value); }

  ModularSetAnalyzerEntry VisitExpr_(const IntImmObj* op) final {
    return ModularSetAnalyzerEntry(0, op->value);
  }
  ...
  ModularSetAnalyzerEntry VisitExpr_(const CDivObj* op) final {
    ModularSetAnalyzerEntry b = VisitExpr(op->b);
    if (b.is_const()) {
  ...
  ModularSetAnalyzerEntry VisitExpr_(const IfExprObj* op) final {
    ModularSetAnalyzerEntry a = VisitExpr(op->then_expr);
    ModularSetAnalyzerEntry b = VisitExpr(op->else_expr);
    return Union(a, b);
  }
  ...
  ModularSetAnalyzerEntry VisitExpr_(const CModObj* op) final {
    ModularSetAnalyzerEntry b = VisitExpr(op->b);
    if (b.is_const()) {
  ...
  ModularSetAnalyzerEntry VisitExpr_(const LShiftObj* op) final {
    ModularSetAnalyzerEntry b = VisitExpr(op->b);
    if (b.is_const() && b.base >= 0) {
      int64_t factor = static_cast<int64_t>(1) << b.base;
      ModularSetAnalyzerEntry a = VisitExpr(op->a);
      return ModularSetAnalyzerEntry(a.coeff * factor, a.base * factor);
    }
    return Everything();
  }

  ModularSetAnalyzerEntry VisitExpr_(const RShiftObj* op) final {
    ModularSetAnalyzerEntry b = VisitExpr(op->b);
    if (b.is_const() && b.base >= 0) {
  ...
  ModularSetAnalyzerEntry VisitExpr_(const BitwiseAndObj* op) final {
    ModularSetAnalyzerEntry b = VisitExpr(op->b);
    if (b.is_const()) {
  ...
  ModularSetAnalyzerEntry VisitExpr_(const CallObj* op) final { return Everything(); }
 ...
 private:
  ...
  std::unordered_map<Var, ModularSetAnalyzerEntry, ObjectPtrHash, ObjectPtrEqual> var_map_;
  /*!
...
ModularSetAnalyzer::ModularSetAnalyzer(AnalyzerObj::Impl* parent)
