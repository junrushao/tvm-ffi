...
struct Comparison {
  ...
  static Comparison NonExist() {
    return Comparison(Key::kNonExist, Key::kNonExist, -1, CompareResult::kInconsistent);
  }
  explicit operator bool() const { return lhs_ != Key::kNonExist; }
...
struct TransitiveComparisonAnalyzer::Impl {
  CompareResult TryCompare(const Expr& lhs, const Expr& rhs,
 ...
 private:
  ...
  std::unordered_map<Expr, Key, ExprStructuralHash, ExprStructuralEqual> expr_to_key;
...
std::tuple<Expr, Expr, int64_t> ExtractOffsets(const Expr& lhs, const Expr& rhs) {
  auto extract_offset = [](const Expr& expr) -> std::pair<Expr, int64_t> {
    ...
    if ((x + c).Match(expr)) {
      ...
      IntImm c_val = c.Eval();
      return {Const(DTypeOf(c_val), 0), c.Eval()->value};
    } else {
      return {expr, 0};
    }
  };
...
TransitiveComparisonAnalyzer::TransitiveComparisonAnalyzer(AnalyzerObj::Impl*)
    : impl_(std::make_unique<Impl>()) {}
TransitiveComparisonAnalyzer::~TransitiveComparisonAnalyzer() = default;

CompareResult TransitiveComparisonAnalyzer::TryCompare(const Expr& lhs, const Expr& rhs,
...
void TransitiveComparisonAnalyzer::Impl::Bind(const Var& var, const Range& range,
  ...
  if (it != prev_bindings_.end()) {
    ...
    bool differs_from_previous = !expr_equal(RangeMin(range), RangeMin((*it).second)) ||
                                 !expr_equal(RangeExtent(range), RangeExtent((*it).second));
    if (differs_from_previous) {
      ...
      if (ExprToPreviousKey(var, &key)) {
        knowns_.erase(std::remove_if(knowns_.begin(), knowns_.end(),
                                     [&](const auto& known) { return known.lhs_ == key; }),
                      knowns_.end());
      }
    }
  }
  ...
  if (IsConstInt(RangeExtent(range), 1)) {
    AddKnown(var == RangeMin(range), &knowns_);
  } else {
    AddKnown(var >= RangeMin(range), &knowns_);
    AddKnown(var < RangeMin(range) + RangeExtent(range), &knowns_);
  }
}

void TransitiveComparisonAnalyzer::Impl::Bind(const Var& var, const Expr& expr,
                                              bool allow_override) {
  Bind(var, Range(expr, Const(DTypeOf(expr), 1)), allow_override);
}
...
std::function<void()> TransitiveComparisonAnalyzer::Impl::EnterConstraint(const Expr& expr) {
  ...
  auto frecover = [old_literal_size, new_literal_size, this]() {
    ...
    using DifferenceType = std::vector<Comparison>::difference_type;
    scoped_knowns_.erase(scoped_knowns_.begin() + static_cast<DifferenceType>(old_literal_size),
                         scoped_knowns_.end());
  };
  return frecover;
}

CompareResult TransitiveComparisonAnalyzer::Impl::TryCompare(const Expr& lhs_expr,
                                                             const Expr& rhs_expr,
                                                             bool propagate_inequalities) const {
  // Currently only supports integer checks
  if (DTypeOf(lhs_expr).code != kDLInt || DTypeOf(rhs_expr).code != kDLInt) {
...
std::vector<Comparison> TransitiveComparisonAnalyzer::Impl::CollectDirectComparisons(
...
std::vector<Comparison> TransitiveComparisonAnalyzer::Impl::CollectIndirectComparisons(
    Key lhs_key, Key rhs_key) const {
...
std::vector<Comparison> TransitiveComparisonAnalyzer::Impl::DFSFromLHS(Key lhs_key,
                                                                       Key rhs_key) const {
  ...
  for (const auto& cmp : lhs_to_rhs) {
    switch (cmp.result_) {
      ...
      case CompareResult::kLT:
        TVM_FFI_THROW(InternalError) << "Normalized comparisons should only include <= and >=";
        TVM_FFI_UNREACHABLE();
      default:
