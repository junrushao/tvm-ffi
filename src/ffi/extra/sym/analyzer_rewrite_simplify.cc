...
std::tuple<Expr, int64_t> ExtractConstantOffset(const Expr& expr) {
  ...
  // check for it.
  if ((x + c1).Match(expr) || (c1 - x).Match(expr)) {
...
CompareResult RewriteSimplifier::Impl::TryCompareUsingKnownInequalities(const Expr& x,
                                                                        const Expr& y) {
...
CompareResult RewriteSimplifier::Impl::TryComparisonOfProductAndSum(const Expr& x, const Expr& y) {
  ...
  auto negate = [](const ConstIntBound& bound) {
...
Expr RewriteSimplifier::Impl::VisitExpr_(const AddObj* op) {
  ...
  if (auto const_res = TryConstFold<AddObj>(op->a, op->b)) return const_res.value();
...
std::function<void()> RewriteSimplifier::Impl::EnterConstraint(const Expr& constraint) {
  ...
  for (const Expr& subconstraint : ExtractConstraints(new_constraint, false)) {
    ...
    Expr negation = [&]() {
      if (DTypeIsBool(DTypeOf(subconstraint))) {
        ...
        // applied.
        return NormalizeBooleanOperators(logical_not(subconstraint));
      } else {
        return subconstraint == Const(DTypeOf(subconstraint), 0);
      }
    }();
    literal_constraints_.push_back(logical_not(negation));
  }
  ...
  auto frecover = [old_literal_size, new_literal_size, this]() {
    ...
    using DifferenceType = std::vector<Expr>::difference_type;
    literal_constraints_.erase(
        literal_constraints_.begin() + static_cast<DifferenceType>(old_literal_size),
...
Expr RewriteSimplifier::Impl::VisitExpr_(const SubObj* op) {
  ...
  op = ret.as<SubObj>();
  if (op == nullptr) return ret;
  if (auto const_res = TryConstFold<SubObj>(op->a, op->b)) return const_res.value();
  // Pattern var to match any expression
  PVar<Expr> x, y, z, b1, b2, s1, s2;
  // Pattern var match IntImm
  PVar<IntImm> c1, c2, c3;
...
Expr RewriteSimplifier::Impl::VisitExpr_(const MulObj* op) {
  ...
  op = ret.as<MulObj>();
  if (op == nullptr) return ret;
  if (auto const_res = TryConstFold<MulObj>(op->a, op->b)) return const_res.value();
  // Pattern var to match any expression
  PVar<Expr> x, y, z, b1, b2, s1, s2;
  // Pattern var match IntImm
  PVar<IntImm> c1, c2;
...
Expr RewriteSimplifier::Impl::VisitExpr_(const CDivObj* op) {
  Expr ret = IRMutatorWithAnalyzer::VisitExpr_(op);
  op = ret.as<CDivObj>();
  if (op == nullptr) return ret;
  if (auto const_res = TryConstFold<CDivObj>(op->a, op->b)) return const_res.value();
  // Pattern var to match any expression
  PVar<Expr> x, y, z, b1;
  // Pattern var match IntImm
  PVar<IntImm> c1, c2, c3;
  ...
  if (const FloatImmObj* ptr = op->b.as<FloatImmObj>()) {
    if (!DTypeIsFloat(DTypeOf(op))) {
      TVM_FFI_THROW(ValueError);
    }
    return op->a * Const(DTypeOf(op->b), 1.0 / ptr->value);
  }
  // Vector rules
  if (DTypeOf(op).lanes != 1) {
    ...
    if ((div(ramp(b1, c1, lanes), broadcast(c2, lanes))).Match(ret)) {
      ...
      // ICHECK(c2val != 0) << "division by zero"; // TODO(junrus): recover
      if (c1val % c2val == 0) {
  ...
  if (IsIndexType(DTypeOf(op))) {
    ...
    if (truncdiv(c1, c2).Match(ret)) {
      int64_t c1val = c1.Eval()->value;
      int64_t c2val = c2.Eval()->value;
      return Const(DTypeOf(op), truncdiv(c1val, c2val));
    }
...
Expr RewriteSimplifier::Impl::VisitExpr_(const CModObj* op) {
  Expr ret = IRMutatorWithAnalyzer::VisitExpr_(op);
  op = ret.as<CModObj>();
  if (op == nullptr) return ret;
  if (auto const_res = TryConstFold<CModObj>(op->a, op->b)) return const_res.value();
  // Pattern var to match any expression
  PVar<Expr> x, y, z, b1;
  // Pattern var match IntImm
  PVar<IntImm> c1, c2;
  ...
  if (IsIndexType(DTypeOf(op))) {
    ...
    TVM_TRY_RECURSIVE_REWRITE_IF(truncmod(x, c1),
                                 truncmod(x, PConst<Expr>(Const(DTypeOf(op), -c1.Eval()->value))),
                                 c1.Eval()->value < 0);
...
Expr RewriteSimplifier::Impl::VisitExpr_(const FloorDivObj* op) {
  ...
  if (op == nullptr) return ret;
  if (auto const_res = TryConstFold<FloorDivObj>(op->a, op->b)) return const_res.value();
  // Pattern var to match any expression
  PVar<Expr> x, y, z, b1;
  // Pattern var match IntImm
  PVar<IntImm> c1, c2, c3;
  ...
  if (IsIndexType(DTypeOf(op))) {
    ...
    if (floordiv(x * c1 + y, c2).Match(ret) || floordiv(x * c1, c2).Match(ret) ||
      ...
      int64_t c1val = c1.Eval()->value;
      int64_t c2val = c2.Eval()->value;
      Expr yval = y.EvalOr(Const(DTypeOf(ret), 0));
...
Expr RewriteSimplifier::Impl::VisitExpr_(const FloorModObj* op) {
  ...
  if (op == nullptr) return ret;
  if (auto const_res = TryConstFold<FloorModObj>(op->a, op->b)) return const_res.value();
  // Pattern var to match any expression
  PVar<Expr> x, y, z, b1;
  // Pattern var match IntImm
  PVar<IntImm> c1, c2;
...
Expr RewriteSimplifier::Impl::VisitExpr_(const MinObj* op) {
  ...
  op = ret.as<MinObj>();
  if (op == nullptr) return ret;
  if (auto const_res = TryConstFold<MinObj>(op->a, op->b)) return const_res.value();
...
Expr RewriteSimplifier::Impl::VisitExpr_(const MaxObj* op) {
  ...
  op = ret.as<MaxObj>();
  if (op == nullptr) return ret;
  if (auto const_res = TryConstFold<MaxObj>(op->a, op->b)) return const_res.value();
...
Optional<Expr> RewriteSimplifier::Impl::TryMatchLiteralConstraint(const Expr& expr) const {
  Expr negation = logical_not(expr);

  ExprDeepEqual expr_equal;
  for (const auto& constraint : literal_constraints_) {
    if (expr_equal(constraint, expr)) {
      return Const(DTypeOf(expr), true);
    }
    if (expr_equal(constraint, negation)) {
      return Const(DTypeOf(expr), false);
    }
  }
  return std::nullopt;
}

Expr RewriteSimplifier::Impl::ApplyRewriteRules(Expr node) {
  if (const auto* op = node.as<EqObj>()) {
    return ApplyRewriteRules(RefFromPtr<Eq>(op));
  }
  if (const auto* op = node.as<LtObj>()) {
    return ApplyRewriteRules(RefFromPtr<Lt>(op));
  }
  if (const auto* op = node.as<NotObj>()) {
    return ApplyRewriteRules(RefFromPtr<Not>(op));
  }
  return node;
}

Expr RewriteSimplifier::Impl::VisitExpr_(const EqObj* op) {
  Expr mutated = IRMutatorWithAnalyzer::VisitExpr_(op);
  if (const EqObj* ret_ptr = mutated.as<EqObj>(); ret_ptr == nullptr) {
    return mutated;
  }
  Eq ret = RefFromPtr<Eq>(mutated.as<EqObj>());
  op = ret.get();
  if (auto const_res = TryConstFold<EqObj>(op->a, op->b)) {
...
Expr RewriteSimplifier::Impl::VisitExpr_(const NeObj* op) {
  ...
  if (IsIndexType(DTypeOf(op->a))) {
    ...
    if (result == CompareResult::kNE || result == CompareResult::kGT ||
      ...
      // False or (b < a)
      // b < a
      return ApplyRewriteRules(less(op->b, op->a));
      ...
      // (a < b) or False
      // a < b
      return ApplyRewriteRules(less(op->a, op->b));
    }
  }

  return ApplyRewriteRules(logical_not(ApplyRewriteRules(equal(op->a, op->b))));
}
...
Expr RewriteSimplifier::Impl::VisitExpr_(const LeObj* op) {
  ...
  if (auto const_res = TryConstFold<LeObj>(op->a, op->b)) return const_res.value();
  if (auto match = TryMatchLiteralConstraint(ret)) return match.value();
  ...
  ret = ApplyRewriteRules(logical_not(ApplyRewriteRules(less(op->b, op->a))));

  op = ret.as<LeObj>();
  if (op && IsIndexType(DTypeOf(op->a))) {
    CompareResult result = TryCompare(op->a, op->b);
    if (result == CompareResult::kLE || result == CompareResult::kLT ||
      ...
      // False or (a == b)
      // a == b
      return ApplyRewriteRules(equal(op->a, op->b));
    }
  }
...
Expr RewriteSimplifier::Impl::VisitExpr_(const GeObj* op) {
  return this->VisitExpr(op->b <= op->a);
}

Expr RewriteSimplifier::Impl::VisitExpr_(const LtObj* op) {
  Expr mutated = IRMutatorWithAnalyzer::VisitExpr_(op);
  if (const LtObj* ret_ptr = mutated.as<LtObj>(); ret_ptr == nullptr) {
    return mutated;
  }
  Lt node = RefFromPtr<Lt>(mutated.as<LtObj>());
  op = node.get();
  if (auto const_res = TryConstFold<LtObj>(op->a, op->b)) return const_res.value();
...
Expr RewriteSimplifier::Impl::VisitExpr_(const NotObj* op) {
  Expr mutated = IRMutatorWithAnalyzer::VisitExpr_(op);
  if (const NotObj* ret_ptr = mutated.as<NotObj>(); ret_ptr == nullptr) {
    return mutated;
  }
  Not ret = RefFromPtr<Not>(mutated.as<NotObj>());
  if (auto const_res = TryConstFold<NotObj>(ret->operand)) return const_res.value();
  if (auto match = TryMatchLiteralConstraint(ret)) return match.value();
...
Expr RewriteSimplifier::Impl::VisitExpr_(const AndObj* op) {
  ...
  if (auto const_res = TryConstFold<AndObj>(op->a, op->b)) return const_res.value();
  if (auto match = TryMatchLiteralConstraint(ret)) return match.value();
  ...
  auto cfalse = PConst<Expr>(Const(DTypeOf(op), false));
...
Expr RewriteSimplifier::Impl::VisitExpr_(const OrObj* op) {
  ...
  Expr ret = [&]() -> Expr {
    ...
    for (size_t i = 0; i < 4; i++) {
      ...
      ConstraintContext context(analyzer_, NormalizeBooleanOperators(logical_not(constraint)));
      Expr updated = VisitExpr(to_update);
  ...
  op = ret.as<OrObj>();
  if (op == nullptr) return ret;
  if (auto const_res = TryConstFold<OrObj>(op->a, op->b)) return const_res.value();
  if (auto match = TryMatchLiteralConstraint(ret)) return match.value();
  if ((enabled_extensions_ & RewriteSimplifier::kConvertBooleanToAndOfOrs) &&
      !recursively_visiting_boolean_) {
    return SimplifyAsAndOfOrs(ret, analyzer_);
  ...
  auto ctrue = PConst<Expr>(Const(DTypeOf(op), true));
...
Expr RewriteSimplifier::Impl::VisitExpr_(const IfExprObj* op) {
  Expr ret = IRMutatorWithAnalyzer::VisitExpr_(op);
  op = ret.as<IfExprObj>();
  if (op == nullptr) return ret;
  // Pattern var to match any expression
  ...
  const Expr& cond = op->cond;
  const Expr& then_expr = op->then_expr;
  const Expr& else_expr = op->else_expr;
  if (const IfExprObj* inner = then_expr.as<IfExprObj>()) {
    const Expr& inner_else_expr = inner->else_expr;
    if (AsConstInt(inner_else_expr) && AsConstInt(else_expr) &&
        analyzer_->CanProveEqual(inner_else_expr, else_expr)) {
      return select(cond && inner->cond, inner->then_expr, else_expr);
    }
  }
  return ret;
}
...
Expr RewriteSimplifier::Impl::VisitExpr_(const CastObj* op) {
  ...
  op = ret.as<CastObj>();
  if (op == nullptr) return ret;
  return cast(DTypeOf(op), op->value);
}
...
RewriteSimplifier::RewriteSimplifier(AnalyzerObj::Impl* parent)
