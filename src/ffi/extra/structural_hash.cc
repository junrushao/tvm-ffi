...
class StructuralHashHandler {
 public:
  ...
  uint64_t HashArray(const Array<Any>& arr) {
    uint64_t hash_value = details::StableHashCombine(arr->GetTypeKeyHash(), arr.size());
    for (const auto& i : arr) {
      hash_value = details::StableHashCombine(hash_value, HashAny(i));
    }
    return hash_value;
  }
  ...
  uint64_t HashMap(const Map<Any, Any>& map) {
  ...
  uint64_t HashTensor(const Tensor& tensor) {
